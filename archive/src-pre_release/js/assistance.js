...
function setTableData(arr) {
	// parseDate
	const serialNumb = parseInt(getHex(arr.slice(0, 4)).join(''), 16);
	const timeStamp = parseInt(getHex(arr.slice(4, 8)).join(''), 16);
	const payload = parseInt(getHex(arr.slice(8, 12)).join(''), 16);
...
function testFunction() {
	...
	function formHexRow(workObj) {
		...
		Object.keys(workObj).forEach(key => {
			...
			mainResArr = mainResArr.concat(...resArr.reverse())
		})
