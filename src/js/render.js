window.addEventListener('load', () => {
  ...
  window.electronAPI.openPortSuccess(() => {
    ...
    const workBtns = ['reg_1_write', 'reg_1_read', 'scan_btn', 'download_btn', 'config_btn']
    workBtns.forEach(btn => {
      const btnDocument = document.getElementById(btn);
  ...
  write_new_config_btn.addEventListener('click', () => writeConfig(true))
})
...
function responseHandler(arr) {
  ...
  if(window.rootObj.scanning && window.rootObj.stageObj.stage2Comp) {
    // отлавливание 12 байтов
    if(arr.length === 12) {
      console.log("TABLE SET_OPTION", arr.length);
      console.log(" %c INPUT TABLE DATA", 'background: #1E90FF; color: #FFFAFA');
      setTableData(arr);
    }
  }
  ...
  if(crc == checkCRCDec) {
    ...
    if(window.readReg === 0x00AA) {
      ...
      (() => {
        ...
        // setDOMConfigValue(window.rootObj.initDevConfData, 'o');
        checkConfigDiffs();
      })();
      ...
      if(stageRunning && !stage1Comp) {
        ...
        console.log(" %c STAGE 1 COMPLETE", 'background: #1E90FF; color: #FFFAFA');
    ...
    if(window.writeReg === 0x00AA) {
      if(functionCode === 0x10) {
        if(window.rootObj.writeNewConfig) {
          window.rootObj.initDevConfData = window.rootObj.devConfData;
          window.rootObj.rowDevConfData = window.rootObj.rowInitDevConfData;
        } else {
          window.rootObj.rowDevConfData = window.rootObj.rowInitDevConfData;
        }
        window.rootObj.writeNewConfig = false;
        debugger
        const config_btn = document.getElementById('config_btn');
        ...
        const write_new_config_btn = document.getElementById('write_new_config_btn');
        write_new_config_btn.style.display = 'none'
    ...
    if(window.writeReg === 0x0066) {
      const { stage1Comp, stage2Comp } = window.rootObj.stageObj
      if(stage1Comp && !stage2Comp) {
        console.log("get 0x0066 res", getHex(arr));
        console.log(" %c STAGE 2 COMPLETE", 'background: #1E90FF; color: #FFFAFA');
        window.rootObj.stageObj.stage2Comp = true;
...
function toggleScan() {
  ...
  if(!scanning) {
    ...
    write_new_config_btn.disabled = true;
  } else {
    ...
    write_new_config_btn.disabled = false;
  }
}
...
function stageHandler() {
  ...
  const pass = [0xF9, 0x03, 0x00, 0xAA, 0x00, 0x02]
  regRead(pass, 0x00AA);
}
...
function setTime(done=false) {
  ...
  if(done) {
    ...
    const minus_secs = window.rootObj.secondsTime;
    // debugger
    const secs = window.rootObj.devConfData.scanTime*60-minus_secs;
    let seconds = secs%60;
    let mins = ~~(secs/60);
...
function writeConfig(newConfig=false) {
  debugger
  const payloadArr = newConfig ? window.rootObj.rowDevConfData : window.rootObj.rowInitDevConfData
  const prev = [0xF9, 0x10, 0x00,  0xAA, 0x00,  0x04, 0x08, ...payloadArr];
  if(!window.rootObj.scanning) {
    window.rootObj.writeNewConfig = newConfig;
    regWrite(prev, 0x00AA);
  } else {
    console.log('CANT WRITE WHILE SCANNING');
  }
